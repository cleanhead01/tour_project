(function ($) {
    ...
    // 스킬 컨테이너가 안보였다가 위치에 도달하면 보이는 방법 228번줄 09.03 (목)
    $('.skillContainer').hide()
    ...
    $(window).scroll(function () {
        ...
        //scrollTop() 값이 100 이상이 되면 맨위로 버튼 보이고, 100미만이면 숨기기
        if (sct >= 100) {
            $('.gotop').addClass('on').stop().animate({ // css에서 준 opacity값의 작은 문제점을 해결
                opacity: '1'
            }, 500)
        } else {
        ...
        if (sct >= $('#skills').offset().top) {
            ...
            // else 위에 조건만 만족되면 되서 else는 없어도 됨
        } else { // 이전 구역에 갔을 때 다시 사라지게 하고 싶을 때
            $('.skillContainer').hide()
        }
